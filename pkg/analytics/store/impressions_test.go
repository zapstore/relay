...
func TestQueryImpressionsSQL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantSQL:  "SELECT COALESCE(SUM(count), 0) AS count FROM app_impressions WHERE day >= ? AND day <= ?",
		...
		},
		{
			...
			wantSQL:  "SELECT app_id, day, COALESCE(SUM(count), 0) AS count FROM app_impressions WHERE day >= ? AND day <= ? GROUP BY app_id, day ORDER BY day DESC",
			wantArgs: []any{"2024-01-01", "2024-01-31"},
		},
		{
			name:     "app_id filter",
			filter:   ImpressionFilter{AppID: "com.example.app", From: "2024-01-01", To: "2024-01-31"},
			wantSQL:  "SELECT COALESCE(SUM(count), 0) AS count FROM app_impressions WHERE app_id = ? AND day >= ? AND day <= ?",
		...
		},
		{
			...
			wantSQL:  "SELECT COALESCE(SUM(count), 0) AS count FROM app_impressions WHERE app_pubkey = ? AND day >= ? AND day <= ?",
			wantArgs: []any{"deadbeef", "2024-01-01", "2024-01-31"},
		},
		{
			name:     "source filter",
			filter:   ImpressionFilter{Source: SourceApp, From: "2024-01-01", To: "2024-01-31"},
			wantSQL:  "SELECT COALESCE(SUM(count), 0) AS count FROM app_impressions WHERE day >= ? AND day <= ? AND source = ?",
			wantArgs: []any{"2024-01-01", "2024-01-31", SourceApp},
		},
		{
			name:     "type filter",
			filter:   ImpressionFilter{Type: ImpressionDetail, From: "2024-01-01", To: "2024-01-31"},
			wantSQL:  "SELECT COALESCE(SUM(count), 0) AS count FROM app_impressions WHERE day >= ? AND day <= ? AND type = ?",
		...
		},
		{
			...
			wantSQL:  "SELECT COALESCE(SUM(count), 0) AS count FROM app_impressions WHERE app_id = ? AND app_pubkey = ? AND day >= ? AND day <= ? AND source = ? AND type = ?",
		...
		},
		{
			...
			wantSQL:  "SELECT day, source, COALESCE(SUM(count), 0) AS count FROM app_impressions WHERE app_id = ? AND app_pubkey = ? AND day >= ? AND day <= ? AND source = ? AND type = ? GROUP BY day, source ORDER BY day DESC",
		...
		},
		{
			...
			wantSQL:  "SELECT app_id, day, COALESCE(SUM(count), 0) AS count FROM app_impressions WHERE app_pubkey = ? AND day >= ? AND day <= ? GROUP BY app_id, day ORDER BY day DESC",
			wantArgs: []any{"deadbeef", "2024-01-01", "2024-01-31"},
		},
		{
			name:     "group by does not mutate caller slice",
			filter:   ImpressionFilter{From: "2024-01-01", To: "2024-01-31", GroupBy: []string{"app_id"}},
			wantSQL:  "SELECT app_id, COALESCE(SUM(count), 0) AS count FROM app_impressions WHERE day >= ? AND day <= ? GROUP BY app_id",
			wantArgs: []any{"2024-01-01", "2024-01-31"},
		},
	}
