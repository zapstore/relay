...
func TestQueryDownloadsSQL(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			filter:   DownloadFilter{},
			wantSQL:  "SELECT COALESCE(SUM(count), 0) AS count FROM app_downloads",
			wantArgs: nil,
		},
		{
			name:     "from and to filters",
			filter:   DownloadFilter{From: "2024-01-01", To: "2024-01-31"},
			wantSQL:  "SELECT COALESCE(SUM(count), 0) AS count FROM app_downloads WHERE day >= ? AND day <= ?",
			wantArgs: []any{"2024-01-01", "2024-01-31"},
		},
		{
			name:     "hash filter",
			filter:   DownloadFilter{Hash: hStr, From: "2024-01-01", To: "2024-01-31"},
			wantSQL:  "SELECT COALESCE(SUM(count), 0) AS count FROM app_downloads WHERE hash = ? AND day >= ? AND day <= ?",
			wantArgs: []any{hStr, "2024-01-01", "2024-01-31"},
		},
		{
			name:     "source filter",
			filter:   DownloadFilter{Source: SourceApp, From: "2024-01-01", To: "2024-01-31"},
			wantSQL:  "SELECT COALESCE(SUM(count), 0) AS count FROM app_downloads WHERE day >= ? AND day <= ? AND source = ?",
		...
		},
		{
			...
			wantSQL:  "SELECT hash, COALESCE(SUM(count), 0) AS count FROM app_downloads WHERE day >= ? AND day <= ? GROUP BY hash",
			wantArgs: []any{"2024-01-01", "2024-01-31"},
		},
		{
			name:     "group by day emits ORDER BY day DESC",
			filter:   DownloadFilter{From: "2024-01-01", To: "2024-01-31", GroupBy: []string{"day"}},
			wantSQL:  "SELECT day, COALESCE(SUM(count), 0) AS count FROM app_downloads WHERE day >= ? AND day <= ? GROUP BY day ORDER BY day DESC",
			wantArgs: []any{"2024-01-01", "2024-01-31"},
		},
		{
			name:     "group by country_code",
			filter:   DownloadFilter{From: "2024-01-01", To: "2024-01-31", GroupBy: []string{"country_code"}},
			wantSQL:  "SELECT country_code, COALESCE(SUM(count), 0) AS count FROM app_downloads WHERE day >= ? AND day <= ? GROUP BY country_code",
			wantArgs: []any{"2024-01-01", "2024-01-31"},
		},
		{
			...
			wantSQL:  "SELECT hash, day, COALESCE(SUM(count), 0) AS count FROM app_downloads WHERE hash = ? AND day >= ? AND day <= ? AND source = ? GROUP BY hash, day ORDER BY day DESC",
